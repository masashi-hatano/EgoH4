...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        q = (
            self.w_q(q)
            .view(bs, n_q, self.n_head, self.d_k)
            .permute(2, 0, 1, 3)
            .contiguous()
            .view(-1, n_q, self.d_k)
        )
        k = (
            self.w_k(k)
            .view(bs, n_k, self.n_head, self.d_k)
            .permute(2, 0, 1, 3)
            .contiguous()
            .view(-1, n_k, self.d_k)
        )
        v = (
            self.w_v(v)
            .view(bs, n_v, self.n_head, self.d_v)
            .permute(2, 0, 1, 3)
            .contiguous()
            .view(-1, n_v, self.d_v)
        )
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, decoder_input, self_attn_time_mask, self_attn_padding_mask):
        ...
        decoder_out, dec_self_attn = self.self_attn(
            decoder_input, decoder_input, decoder_input, mask=self_attn_time_mask
        )
...
class CrossDecoderLayer(nn.Module):
    ...
    def forward(
        self, decoder_input, k_input, self_attn_time_mask, self_attn_padding_mask
    ):
        # decode_input: BS X T X D
        # k_input: BS X K X D
        # time_mask: BS X T X T (padding postion are ones)
        ...
        decoder_out, dec_self_attn = self.self_attn(
            decoder_input, k_input, k_input, mask=self_attn_time_mask
        )
        # BS X T X D, BS X T X T
        decoder_out *= self_attn_padding_mask.unsqueeze(-1).float()
        # BS X T X D
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.start_conv_obs = nn.Conv1d(d_feats + d_cond, d_model, 1)
        self.start_conv_fut = nn.Conv1d(d_feats, d_model, 1)
        self.mlp = Mlp(in_features=384, hidden_features=d_model // 4, out_features=d_model // 4)
        # self.start_conv_wrist = nn.Conv1d(4, d_model, 1)
        ...
        self.layer_stack = nn.ModuleList(
            [DecoderLayer(d_model, n_head, d_k, d_v) for _ in range(n_layers)]
        )

        self.use_full_attention = use_full_attention

    def forward(self, decoder_input, imgs_feat, padding_mask, decoder_pos_vec, obj_embedding=None):
        # decoder_input: BS X D X T
        # imgs_feat: BS X T X Dv (Dv is the dimension of visual features)
        ...
        input_embedding_obs = self.start_conv_obs(decoder_input[:, :, :20])
        input_embedding_fut = self.start_conv_fut(decoder_input[:, :self.d_feats, 20:])  # only take noise part (:self.d_feats) for forecasting
        input_embedding_obs = input_embedding_obs.transpose(1, 2)  # BS X T X D
        input_embedding_fut = input_embedding_fut.transpose(1, 2)  # BS X T X D

        # concatenate
        if imgs_feat is not None:
            imgs_feat = self.mlp(imgs_feat)  # BS X T X D
            input_embedding_obs = torch.cat((input_embedding_obs, imgs_feat), dim=-1)  # BS X T X 2D
            input_embedding_obs = self.fuse(input_embedding_obs)  # BS X T X D

        input_embedding = torch.cat((input_embedding_obs, input_embedding_fut), dim=1)  # BS X T X D

        # noise_t_embedding
        if obj_embedding is not None:
            new_input_embedding = torch.cat(
                (obj_embedding, input_embedding), dim=1
            )  # BS X (T+1) X D
        else:
